...
//Circle has 10,000 US Dollars in the bank.
//Circulating supply of USDC on Mina cannot be more than 10,000.
//Example: 8,000 USDC (current circulating supply). They can mint at most 2,000 USDC.
export class FungibleToken extends TokenContract {
   ...
   * @argument {PublicKey} oraclePubKey - the public key of the oracle that provides the amount of reserves in the bank for the stablecoin issuer.
  */
  @method
  async initialize(
    admin: PublicKey,
  ...
  async mint(recipient: PublicKey, amount: UInt64, reserve: UInt64, signature: Signature): Promise<AccountUpdate> {
    ...
    const circulating_supply = await this.getCirculating();
    const supply_after_mint = circulating_supply.add(amount);
    supply_after_mint.assertLessThanOrEqual(reserve);
    const signature_message = reserve.toFields()
    signature_message.push(nonce)
    const validSignature = signature.verify(oraclePublicKey, signature_message);
    validSignature.assertTrue(); 
    this.nonce.set(nonce.add(1))
    //Stablecoin end
